...
public class NearbyWords implements SpellingSuggest {
	...
	public void insertions(String s, List<String> currentList, boolean wordsOnly ) {
		for(int i = 0; i<=s.length(); i++){
			for(int charCode = (int)'a'; charCode <= (int)'z'; charCode++){
				...
				str = str.insert(i, (char)charCode);
	...
	public void deletions(String s, List<String> currentList, boolean wordsOnly ) {
		for(int i=0; i<s.length(); i++){
			...
			if(!currentList.contains(str.toString()) && (!wordsOnly||dict.isWord(str.toString())) && !s.equals(str.toString())){
				currentList.add(str.toString());
			}
		}
	}
	...
	public List<String> suggestions(String word, int numSuggestions) {
		...
		while(!queue.isEmpty() && (numSuggestions > suggestions)){
			String curr = queue.remove(0);
			List<String> Mutations = distanceOne(curr, true);
			
			for(String mutation : Mutations){
				if(!visited.contains(mutation) && !queue.contains(mutation)){
					queue.add(mutation);
