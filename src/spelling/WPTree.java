...
public class WPTree implements WordPath {
	...
	public List<String> findPath(String word1, String word2) 
	{
		...
		while(!queue.isEmpty() && !queue.contains(word2)){
			WPTreeNode curr = queue.remove(0);
			List<String> words = this.nw.distanceOne(curr.getWord(), true);
			for(String word : words){
				if(!visited.contains(word)){
					curr.addChild(word);
					visited.add(word);
					WPTreeNode next = new WPTreeNode(word, curr);
					queue.add(next);
					
					if(word.equals(word2)){
						retList = next.buildPathToRoot();
